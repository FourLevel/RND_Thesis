...
def process_single_expiration(expiration_date):
    try:
        ...
        print(f"處理到期日 {expiration_date} 時發生錯誤: {str(e)}")
        return None

# 主程式
def main():
    # 讀取所有不重複的到期日
    unique_expirations = pd.to_datetime(instruments['date']).dt.strftime('%Y-%m-%d').unique()
    ...
    for exp_date in unique_expirations:
        print(f"處理到期日: {exp_date}")
        result = process_single_expiration(exp_date)
        if result:
    ...
    # 儲存結果
    df_results.to_csv('RND_all_expirations_results.csv', index=False)
    print(f"\n處理完成! 共處理 {len(all_results)} 個到期日")
    print("結果已儲存至 RND_all_expirations_results.csv")
