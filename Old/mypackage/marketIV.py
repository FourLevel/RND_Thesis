...
def hello():
    print("Hello, this is the test function!")
...
# (3.5) mix_cp_function V2
def mix_cp_function_v2(date, call_strike, put_strike, df_idxprice, df_futuresprice, expiration_date):
    formatted_date = datetime.strptime(date, "%Y-%m-%d").strftime("%d%b%y").upper()
    call_iv = pd.read_csv(f"deribit_data/iv/call/call_iv_{formatted_date}.csv", index_col="Unnamed: 0")
    put_iv = pd.read_csv(f"deribit_data/iv/put/put_iv_{formatted_date}.csv", index_col="Unnamed: 0")
